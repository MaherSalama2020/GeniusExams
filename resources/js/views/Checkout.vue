  ...
  <div class="container">
    <div class="row">
      <div class="col-md-8 offset-md-2">
        <div class="order-box">
          <img class="col-md-12" :src="product.image" :alt="product.name" />
          <h2 class="title" v-html="product.name"></h2>
          <p class="small-text text-muted float-left">$ {{product.price}}</p>
          <p class="small-text text-muted float-right">Booked: {{product.bookedNO}}</p>
          <br />
          <hr />
          <!-- <label class="row">
...
export default {
  ...
  methods: {
    ...
    placeOrder(e) {
      ...
      axios.post(`api/products/bookedno`, { product_id }).then((response) => {
        this.product.bookedNO = this.product.bookedNO + 1;
        this.$forceUpdate();
      });
